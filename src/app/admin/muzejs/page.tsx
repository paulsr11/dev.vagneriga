...
export default function MuzejsAdmin() {
  const [content, setContent] = useState({
    title: "Muzejs",
    subtitle: "Vāgnera nams",
    ...
    tickets: [
      { type: "Pieaugušo biļete", price: "EUR 10.00" },
      { type: "Studentu biļete", price: "EUR 10.00" },
      { type: "Grupas biļete (no 8 cilvēkiem)", price: "EUR 10.00" }
    ]
  });
    ...
    <div className="min-h-screen bg-gray-50 p-8 pt-32">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="bg-zinc-900 p-8 text-white flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Muzeja lapa</h1>
        ...
        <div className="p-8 space-y-10">
          ...
          {/* Tickets Section */}
          <section>
            <h2 className="text-xl font-bold border-b pb-4 mb-6 text-zinc-800">Biļešu cenas</h2>
            <div className="space-y-4">
              {content.tickets.map((ticket, index) => (
                <div key={index} className="grid grid-cols-2 gap-4 p-4 bg-zinc-50 rounded-xl border border-zinc-100">
                  <div>
                    <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">Biļetes veids</label>
                    <input 
                      type="text" 
                      value={ticket.type}
                      onChange={(e) => {
                        const newTickets = [...content.tickets];
                        newTickets[index].type = e.target.value;
                        setContent({...content, tickets: newTickets});
                      }}
                      className="w-full p-2 border border-zinc-200 rounded-lg outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">Cena</label>
                    <input 
                      type="text" 
                      value={ticket.price}
                      onChange={(e) => {
